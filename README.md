...
```
cd examples/rknn_yolo26_demo_dfl_open
...
&emsp;&emsp;后处理中代码对于输出80个类别，同样先选择最大类别值，然后再进行反量化，最后只对这一个类别值进行sigmoid，这样后处理的时耗加速很多。最初也是按照对类别每个输出值都反量化和sigmoid，再判断是否大于阈值，测试后处理时耗30多ms，进行先选最大
